...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        public static IEnumerable<Emp> Task2()
        {
            IEnumerable<Emp> result =
                from emp in Emps
                where emp.Salary is > 3 && emp.Job == "Frontend programmer"
        ...
        public static IEnumerable<object> Task7()
        {
            IEnumerable<object> result =
                ...
                select new { NumberOfEmployees = g.Count(), Job = g.Key };
            return result;
        }
        ...
        public static bool Task8()
        {
            bool result = (
                (
                    from emp in Emps
                    where emp.Job.Equals("Backend programmer", StringComparison.OrdinalIgnoreCase)
                    select emp
                ).Any()
            );
            return result;
        ...
        public static Emp Task9()
        {
            Emp result = (
                from emp in Emps
                where emp.Job.Equals("Frontend programmer", StringComparison.OrdinalIgnoreCase)
        ...
        {
            IEnumerable<object> toBeJoined = (
                from nothing in Enumerable.Empty<object>()
                select new
                {
                    Ename = "No Value",
                    Job = (string)null,
        ...
        {
            IEnumerable<Emp> result = Emps.GetEmpsWithAtLeast1DirectSubordinate();
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = from dept in Depts from emp in Emps select dept;
            //result =
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            var result = emps.Where(e => emps.Any(e2 => e2.Mgr == e.Mgr))
                .OrderBy(e => e.Ename)
        ...
        /// The method should return only those employees who have at least 1 direct subordinate.
        /// Employees should be sorted within the collection by surname (ascending) and salary (descending).
        public static IEnumerable<Emp> GetEmpsWithAtLeast1DirectSubordinate(
            this IEnumerable<Emp> emps
        )
        {
            var result =
                from emp in emps
                where (from e2 in emps where e2.Mgr == emp.Mgr select emps).Any()
        ...
        {
            Console.WriteLine($"---Task {++LinqTasks.Counter}---");
